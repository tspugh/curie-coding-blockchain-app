...
- **R8 (MUST — settlement)** *(Amended by [`docs/amendments/0008-real-escrow-settlement.md`](../amendments/0008-real-escrow-settlement.md): settlement is now a **real escrow release**, not an event marker.)* The **insurer deposits `requestedAmount` into escrow at `insurerEngage`** (now `payable`). On `settle` from `Approved`, the contract **transfers `coveredAmount` → provider and refunds `escrowAmount − coveredAmount` → insurer**; from `Denied` it refunds the full escrow. Every terminal-non-settle outcome (`Deadlocked`, `ProviderRefused`, `PolicyInvalidated`, `Withdrawn`) refunds the full escrow to the insurer. The agent-fee float (R9) is tracked separately and never commingled with escrow. `Settled(reqId, coveredAmount, refundedToInsurer)` is emitted. Value-moving paths use CEI + `nonReentrant` (pull-over-push preferred — SPEC-0006 R-OPEN-9). *(Superseded text: "v0 is an event marker only (no token transfer)"; retained as the rollback fallback behind an `escrowEnabled` flag per Amendment 0008.)*
