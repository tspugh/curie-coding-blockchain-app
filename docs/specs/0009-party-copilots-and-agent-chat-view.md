...
Status: Approved (2026-06-07) · Owner: tspugh · Date: 2026-06-06 · Builds on: [SPEC-0001](0001-mvp0-coverage-negotiation.md), [SPEC-0003](0003-token-flow-visibility.md), [SPEC-0004](0004-data-and-evidence-model.md), [SPEC-0006](0006-somnia-agent-platform-integration.md), [SPEC-0007](0007-clause-typed-policy-adjudication.md) (clause-typed policy + attestations), [SPEC-0008](0008-wallet-onboarding-modal.md) (wallet onboarding / BYOK) · Anticipates the v1 "autonomous policy-driven party agents" deferred in [SPEC-0001 §7](0001-mvp0-coverage-negotiation.md)
...
- `web/src/components/CopilotPane.tsx` (new) — the primary co-pilot chat: four-type message stream + glyphs (R28/R29), live in-place status line + thinking/streaming (R29a), input box (R32), auto toggle + kill-switch (R33). *(Hand-rolled React — CopilotKit not used; OQ-7 = A.)*
...
- `web/src/hooks/useNarration.ts` (new) — consumes the narration stream (R17), reconciles `chain` blurbs against on-chain events by tx hash. *(Hand-rolled SSE/JSON; OQ-7 = A.)*
...
- **CopilotKit — evaluated, NOT adopted for v0 (OQ-7 = hand-roll).** The chat UI is **hand-rolled in React** against our own Lambda (SSE/JSON streaming) — **no CopilotKit dependency, no Copilot Cloud, no LangGraph Platform**. CopilotKit (self-hosted, or its free Cloud Developer tier) stays a documented later swap if the hand-rolled UX proves costly. Rationale: [`../research/copilotkit-fit-for-spec-0009.md`](../research/copilotkit-fit-for-spec-0009.md).
...
7. **OQ-7 (use CopilotKit?) — RESOLVED → (A) hand-roll (2026-06-07).** The chat UI is **hand-rolled in React** (the four-type stream + glyphs + status/thinking/streaming + input + auto/kill-switch + collapsible contract drawer — the mockup is the visual spec) talking to **our own Lambda** over SSE/JSON; the LangGraph/Bedrock brain runs in that Lambda; party actions are executed and **signed in the browser** (SPEC-0008 BYOK). **No CopilotKit dependency, no Copilot Cloud, no LangGraph Platform** — simplest + self-contained. (Copilot Cloud's free Developer tier — 50 MAU / 200 threads / 3-day retention — and self-hosted CopilotKit remain documented fallbacks if the hand-rolled UX gets expensive: [`../research/copilotkit-fit-for-spec-0009.md`](../research/copilotkit-fit-for-spec-0009.md).)
