...
- **R30 (MUST — agent-as-right-drawer + form canvas; co-pilot model).** The **agent companion is a collapsible right-side drawer**; the **case/form surface (real action buttons + live on-chain data) is the main canvas and the single source of UI state**, reusing the existing `Detail.tsx` / `Create.tsx` / `Network.tsx` surfaces. The form state is **always maintained behind the scenes** and is populated by either the agent (co-pilot) or the human. Behavior:
  - **Agent mode ON:** the companion drawer is open on the right; the agent fills the form on the canvas; the operator watches fields populate and may edit/override before submit.
  - **Agent mode OFF (manual):** the companion collapses and the **form canvas is shown by default** (auto-revealed), so a user can operate the protocol with no agent at all.
  - Each chat blurb keeps a "show technical" affordance that focuses the canvas on that exact event's detail (on-chain receipt, state transition, ruling verdict, evidence refs, tx monitor).
- **R30a (MUST — co-pilot, human override).** The agent populating the form MUST NOT auto-submit in manual mode (the R15 interrupt still applies); the human may accept, edit, or discard what the agent filled. Turning the agent off at any point leaves the partially-filled form usable manually. The form is the shared state; agent and human are interchangeable editors of it.
