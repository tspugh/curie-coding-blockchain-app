...
6. **OQ-6 (memory persistence) — DIRECTION SET; lean DynamoDB (2026-06-07).** Copilot memory + standing rules are **persisted** off-chain (R12a). **Lean: DynamoDB** — effectively **free** at our volume: storage is free under the **25 GB** free tier (we store kilobytes), requests are **$1.25 / million writes + $0.25 / million reads** on-demand (or **$0** under the provisioned 25-WCU/25-RCU free tier ≈ ~200M req/mo). Decisively, DynamoDB has **no idle/baseline cost**, unlike **RDS MySQL** (~$12+/mo always-on) or **Aurora Serverless v2** (min-ACU baseline) — so it wins the "must be cheap" test. SQL alternatives if needed: PlanetScale serverless MySQL (free tier) or Supabase Postgres. De-identified/synthetic only; **never on-chain**. ([DynamoDB pricing](https://aws.amazon.com/dynamodb/pricing/on-demand/).)
7. **OQ-7 (use CopilotKit?) — priority: medium; lean revised 2026-06-07.** CopilotKit's *frontend* is a clean fit, but its **backend** (CopilotRuntime + CoAgents + AG-UI streaming) leans on **managed services** (Copilot Cloud + LangGraph Platform) and runs against the Lambda grain (research: [`../research/copilotkit-fit-for-spec-0009.md`](../research/copilotkit-fit-for-spec-0009.md) §Hosting). **Revised lean: hand-roll a simple chat UI on our own plain Lambda** (LangGraph-JS in-process + our own SSE/JSON narration stream + browser-side signing) — no CopilotKit backend, no managed deps, no new infra beyond the Lambda; the mockup already specifies the UX. CopilotKit-UI is an optional later add (self-hosted only, after a Lambda-streaming spike); **avoid Copilot Cloud / LangGraph Platform for v0.**
