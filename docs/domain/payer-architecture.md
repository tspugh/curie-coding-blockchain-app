...
**Product context:** Curie Negotiation Protocol mediates parts of the **coverage-decision and fund-release flow** between providers and payers. Understanding which payer subsystem owns which decision is foundational to the on-chain protocol design — the on-chain mediator consumes outputs from several of these systems but should not re-implement them.
