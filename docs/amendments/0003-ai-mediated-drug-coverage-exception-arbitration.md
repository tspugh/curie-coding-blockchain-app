...
Prior authorization has the ideal back-and-forth *shape* (request → "insufficient, justify" → rebut → rule → appeal → settle) but is the most PHI-soaked process in healthcare and usually happens as queued staff work, paperwork routing, and patient follow-up rather than a live doctor-versus-insurer exchange. **We borrow its asynchronous shape and make every object public.**
...
**Vertical: drugs.** Public anchors are clean — published formularies, public price refs (NADAC, Mark Cuban Cost Plus, GoodRx, Medicare/Medicaid dashboards), drug identity via **RxNorm/NDC (free)**, and public drug-level evidence (clinical guidelines, **openFDA/DailyMed** labels, comparative-effectiveness). Drug formulary-exception / step-therapy / tier-exception is the most structured, repeatable PA evidence path. (Procedures rejected: CPT is AMA-copyrighted, criteria are more PHI-bound.)

**Actors.** Provider-side PA team/agent (requests coverage/exception) ↔ Payer-side UM workflow/agent = **insurer/plan** (whose published formulary sets the criteria) ↔ **AI mediator** (adjudicates each round).
...
The back-and-forth comes from the lifecycle; the agent is invoked each round. The demo can compress the timeline, but the product model is asynchronous case progression, not a requirement that clinicians and payer staff work the case together live.

**What the AI actually does (the irreplaceable judgment):** weigh a **free-text clinical-evidence submission** against **free-text formulary criteria**. The formulary says, in prose, e.g. *"non-preferred; covered by exception on documented failure/contraindication of two preferred alternatives per guideline."* The provider cites public evidence; the mediator reads both and rules `approve | deny | need_more_evidence` with a rationale. This is text-vs-text reasoning a script cannot do.
...
- All cases **synthetic and non-identifying**; submissions stay at the drug + formulary + public-evidence level — never a patient chart.
