...
**Last updated:** 2026-05-29 (tick 41 — fix R2b self-contract revert in simulated mode)
**Current mode:** `impl`
**Current tick:** 41
**Last focus:** UNIT-fix-react-submit-click-workaround turned into UNIT-fix-r2b-simulated-self-contract — tick-39's "agent-browser click doesn't fire onSubmit" was a misdiagnosis. The click DID fire; the contract reverted with `"create: self-contract"` because simulated mode shares the providerClient/insurerClient instance (tick-25 MEDIUM 1 closure) so `INSURER_ADDRESS === providerClient.wallet.address` trips R2b. Fix: `web/src/client.ts` exports a fixed synthetic counterparty (`0x…c0c0c0`) when `!IS_REAL` (sim backend doesn't authenticate msg.sender). Real mode unchanged. Harness: 12/35 → **13/35**. Next blocker uncovered: clicking `engage-load-compliant` doesn't make `engage-submit` button render (React re-render timing OR canEngage predicate unmet). Queued as UNIT-engage-flow-silent-fail.
**Last commit:** `<this tick>` (tick 41 — R2b synthetic insurer)
...
### UNIT-engage-flow-silent-fail (NEW — tick 41 finding)
**After harness creates a request + switches to insurer profile, clicking `engage-load-compliant` doesn't make `engage-submit` button render**
- Files: `web/src/views/Detail.tsx` (lines 555-605 — `canEngage` predicate + `policyChoice` state)
- R-citations: SPEC-0001 R5/T3 (adjudication gated until policy attached), browser-verify tick 41
- Acceptance criterion: in simulated mode, after `create-submit`, `profile-pill-insurer`, and `engage-load-compliant`, the `engage-submit` button is in the DOM and clickable. Scenario A reaches `state == Ready` after `engage-submit` click. Two leads: (a) `canEngage = state === State.Open && isInsurer` might evaluate false because `isInsurer = partyId === n.insurerId` and the harness's `n.insurerId` could be different from the active profile's partyId. (b) React's re-render after policyChoice state update may not fire before agent-browser tries to click the (not-yet-rendered) `engage-submit`.
