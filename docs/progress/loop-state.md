...
**Last updated:** 2026-05-29 (tick 40 — fix-e2e-harness-api-shape + profile-pill testids)
**Current mode:** `impl`
**Current tick:** 40
**Last focus:** UNIT-fix-e2e-harness-api-shape — exposed `window.__curie.{negotiation,content,wallet,profiles}` via active-client Proxy getters; gated by `import.meta.env.DEV || VITE_EXPOSE_TEST_API==="1"` so production preview builds opt-in. Bundled with a 1-line profile-pill `data-testid="profile-pill-{id}"` addition + run.sh rewrite to click those testids. Harness now **12/35 pass** (was 9/35). Scenario D 4/4 PASS, Scenario G 2/3 PASS. Remaining 23 failures all downstream of the React-onSubmit click-bug (UNIT-fix-react-submit-click-workaround). Strict-review PASS zero findings.
**Last commit:** `<this tick>` (tick 40 — e2e harness API shape)
