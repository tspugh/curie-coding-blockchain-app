...
## Tick 57 strict-review

**Date:** 2026-05-30
**Scope:** the 2 files changed this tick:
- `docs/progress/coverage.md` — APPENDED `## Tick 57 — full src/ coverage measurement` section + per-module table + queued P1–P7 gaps; verdict FAIL.
- `src/config/networks.test.ts` (NEW, 76 LOC, 6 tests) — pins `SOMNIA_TESTNET` / `SOMNIA_MAINNET` / `SOMNIA_NETWORKS` constants + `txUrl` / `addressUrl` builders + trailing-slash normalization contract.

Closes the P7 gap from the same tick's own measurement (`src/config/networks.ts` 0% → 100%).

R-citations the unit claims to satisfy: **none required** — `networks.ts` is infra/config, not spec-numbered. Per loop convention, this is the correct stance.

### Scrutiny checklist (per tick-57 prompt)

| # | Item | Finding |
|---|------|---------|
| 1 | **Pinned values vs impl** — chain ids, RPC/WS/explorer/faucet URLs in the test must match `src/config/networks.ts` L28-46 byte-for-byte. | **Clean cross-check.** Testnet: chainId `50312` (L30 ✓), currency `STT` (L31 ✓), rpc `https://api.infra.testnet.somnia.network/` (L32 ✓), ws `wss://api.infra.testnet.somnia.network/ws` (L33 ✓), explorer `https://shannon-explorer.somnia.network/` (L34 ✓), faucet `https://testnet.somnia.network/` (L35 ✓). Mainnet: chainId `5031` (L40 ✓), currency `SOMI` (L41 ✓), rpc/ws/explorer (L42-44 ✓). One non-finding: mainnet `faucetUrl` (`https://stakely.io/faucet/somnia-somi`, L45) is **not** pinned. Defensible — the mainnet faucet URL is an external third-party endpoint (`stakely.io`) whose stability the app cannot guarantee; pinning it would create churn out of the project's control. The testnet faucet (an official `somnia.network` URL) **is** pinned, which is the higher-value contract. Scope-trim is honest and asymmetric for a defensible reason. No finding. |
| 2 | **Trailing-slash test genuine** — does the synthetic `fakeNetwork` test exercise `.replace(/\/$/, "")` in a way that would catch impl drift to e.g. `endsWith("/") ? slice(0,-1) : raw`? | **Pins behavior, not implementation — which is the correct thing to pin.** The fake-network case (`"https://no-trailing-slash.example"` — no trailing slash) verifies that the builder produces a single `/` between base and path WHEN the base lacks a trailing slash. The mainnet/testnet cases (both have a trailing slash) verify the same invariant under the trailing-slash branch. Together they pin the contract "exactly one `/` between base and the `tx/`/`address/` segment" across both branches of the regex/ternary. An impl drift to `endsWith("/") ? slice(0,-1) : raw` would still pass these tests, which is **correct** — the test should pin behavior (no double-slash, no missing-slash), not the regex form. Not pinned: the multi-trailing-slash case (`"https://x.com//"`), which the regex strips only one of. The L64 comment ("strip exactly one trailing slash") even calls this out honestly. Scope-trim is defensible. No finding. |
| 3 | **Type strictness** — `{ ...SOMNIA_TESTNET, explorerUrl: "https://..." }`: does this satisfy `SomniaNetwork`'s `readonly` modifiers, or is there a type-coercion smell? | **Standard TS structural compat, no coercion.** Spreading a `readonly`-fielded interface into a fresh object literal produces a mutable object that is structurally assignable to `SomniaNetwork` because `readonly` is enforced at the binding site, not the value. The fresh literal is bound to `txUrl`'s `network: SomniaNetwork` parameter, which only reads `explorerUrl` — never writes. No `as`-cast, no `any`. The override (`explorerUrl: "https://no-trailing-slash.example"`) is a `string`, matching the field type. Clean. No finding. |
| 4 | **Comment quality** — file header + per-test comments: any that lie or restate the code? | **All load-bearing, none lie.** Header (L1-8) explains *why* the test exists (prevents wallet-chip / explorer-link drift) — not restatement. L48 comment ("Mainnet path verifies the trailing-slash normalization works the same way") is accurate and explains why mainnet is exercised under `txUrl` instead of `addressUrl`. L64 comment ("Both functions use the same `.replace(/\/$/, "")` — pin that contract") is accurate and load-bearing — it tells the future reader the test is pinning a shared regex contract. No restate-the-code comments. No finding. |
| 5 | **No-mock principle / identity check** — is `SOMNIA_NETWORKS.testnet === SOMNIA_TESTNET` actually identity-checked, not deep-equal? | **Identity, via strict equality.** Test imports `assert from "node:assert/strict"` (L9). `assert/strict`'s `equal` uses `Object.is` — strict reference equality for objects. The assertion `assert.equal(SOMNIA_NETWORKS.testnet, SOMNIA_TESTNET)` therefore fails if the registry rebuilds the constant rather than re-exporting the same reference. This pins the `as const`-frozen identity contract that `src/config/networks.ts` L48-51 establishes. No re-declaration of the constants in the test — they are imported and identity-checked. Clean. No finding. |
| 6 | **Test coverage of the new test file itself** — `addressUrl` is only exercised with testnet, not mainnet; defensible scope-trim or finding? | **Defensible scope-trim.** The test exercises (a) `txUrl` with testnet + mainnet + fake, (b) `addressUrl` with testnet + fake. The "mainnet × addressUrl" cell is the only one not directly exercised. Both functions share the same `.replace(/\/$/, "")` base-URL handling (verified L57 vs L62 of impl — identical except for the `/tx/` vs `/address/` literal). Mainnet's URL behavior is pinned via `txUrl`; the `/address/<addr>` path-segment behavior is pinned via testnet's `addressUrl` and the fake-network case. The missing cell adds no new branch coverage — it would only re-verify the cross-product. Confirmed by running `node --experimental-test-coverage`: `networks.ts` reaches **100% line / 100% branch / 100% function** with the existing 6 tests. Not a finding. |
| 7 | **Coverage.md fidelity** — does the new section's raw table actually match what `node --experimental-test-coverage` produced before the new test landed? | **Faithful.** The markdown's "Raw tool output" shows aggregate `98.01 / 87.50 / 91.58` with `networks.ts` absent from the table (correct — pre-tick state, no test imported it). I independently re-ran `node --experimental-test-coverage` with the new test present: aggregate became `98.08 / 87.91 / 91.94` and `networks.ts` now appears at `100/100/100`. The delta is consistent with adding ~30 covered LOC across 5 statements + branches. The pre-tick numbers in the markdown are internally consistent and reproducible. The per-module verdicts (`simulated.ts` 68.63% branch, `profiles.ts` 84.92% line, the seven 0% modules) all match what the tool produced. No fudged numbers. |
| 8 | **R-citations** — should there be any in the new file? | **None, correctly.** `src/config/networks.ts` is infra/config — not a spec-numbered surface. The file has no R-tags, and the test file has none either. Consistent with how other infra-test files in this branch are scoped. No finding. |
| 9 | **Carried-forward gates** — does `npm run test:lib` actually report 91/91, and does the new file participate in the existing glob? | **Confirmed.** Re-ran `npx tsx --test "src/**/*.test.ts"` — observed `# tests 91 # pass 91 # fail 0` (matches the claim of 91/91, +6 over the prior 85/85). The glob picks up `src/config/networks.test.ts` automatically; no test-runner config changes needed. No new dependencies — `node:assert/strict` and `node:test` are stdlib, the import is from `./networks.js` (matches the existing `.js`-extension convention for tsx). |
| 10 | **Coverage.md verdict honesty** — the markdown declares FAIL despite the tool-measured aggregate being 98%+. Is this self-FAIL honest or theatrical? | **Honest.** The markdown explicitly separates "tool-measured aggregate" (98.01% / 87.50%, above threshold) from "holistic including untested modules" (estimated ~55% / ~45%). The FAIL is anchored on (a) seven modules at 0% the tool doesn't surface and (b) `simulated.ts` branch 68.63% being below the 85% threshold *within* the tool-measured set. Both are real, reproducible gaps. The P1-P7 queue with acceptance criteria turns the FAIL into actionable next work rather than a despairing footnote. The verdict isn't theatrical — the threshold genuinely isn't met holistically, and the markdown distinguishes the two views cleanly. |

### New findings

**None.** Each scrutiny item resolved cleanly with in-source or in-run evidence.

### Empirical re-verification

- `npx tsx --test "src/**/*.test.ts"` → **91/91 pass** (matches the +6 claim over tick-56's 85/85 baseline).
- `npx tsx --test --experimental-test-coverage "src/**/*.test.ts"` → `networks.ts` now reports **100.00% line / 100.00% branch / 100.00% function**, closing the P7 gap from the same tick's own measurement.
- Hardhat 30/30, harness 37/37, root + web `tsc` clean — carried forward; no contracts, no web/UI, no shared TS surface touched this tick.

### Verdict: **PASS**

The new test pins the right contracts (chain ids, RPC/WS/explorer URLs, registry identity, trailing-slash normalization across both branches), uses identity-checking from `node:assert/strict` correctly, and scope-trims only the cells that add no new coverage (mainnet × addressUrl, multi-trailing-slash, mainnet faucet). The coverage.md section is empirically reproducible and the self-FAIL verdict is honest — anchored on real zero-coverage modules and a real sub-threshold branch in `simulated.ts`. P1-P7 queue gives the next ticks a concrete work plan. Tick 57 closes one gap (P7) and names the rest; fit to commit.

## Tick 58 strict-review

**Date:** 2026-05-30
**Scope:** 1 new file:
- `src/content/content.test.ts` (149 LOC, 15 tests) — closes the P2 gap from tick 57's coverage queue (`src/content/content.ts` was at 0%).

R-citations the unit claims to satisfy: **SPEC-0001 R3, R4, T1**. All three are present in `docs/specs/0001-mvp0-coverage-negotiation.md` (R3 line 49, R4 line 50, T1 line 208) and the test header at L4-6 cites them accurately. R3 ("both parties can verify their copy matches") is the verification helpers (`ContentStore.verify`, `verifyContent`); R4 (hash-only-on-chain) is pinned by testing that `hashContent` is the on-chain wire format, never the content. T1 cross-references R3+R4 in the spec's T-table and is correctly invoked.

### Scrutiny checklist (per tick-58 prompt)

| # | Item | Finding |
|---|------|---------|
| 1 | **R3/R4 spec citation accuracy** — do the comment header + per-test cites map to actual spec lines? | **Accurate.** `docs/specs/0001-mvp0-coverage-negotiation.md` L49 defines R3 (off-chain content, hash-on-chain, both-party verification), L50 defines R4 (no PHI/content beyond hashes etc. ever on-chain), L208 defines T1 as `(R3,R4)`. Test header L4-6 cites SPEC-0001 R3/R4/T1; test names cite R3 at L106 ("confirms matching content against on-chain hash (R3)") and R4 at L9-11 ("HARD INVARIANT (R4): the on-chain commitment is the hash, never the content"). No misattribution. No finding. |
| 2 | **Empty-string keccak literal** — `0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470`. | **Verified independently.** Ran `node -e "const {ethers}=require('ethers'); console.log(ethers.keccak256(ethers.toUtf8Bytes('')))"` → exact match. This is the canonical empty-bytes keccak256; pinning it catches a future swap of the hash primitive (e.g. SHA-3 vs Keccak-256 variant confusion). No finding. |
| 3 | **Independent computation test (test 1) — self-pinning?** L24-29 calls `ethers.keccak256(ethers.toUtf8Bytes(text))` and compares to `hashContent(text)`. The impl L20 is exactly `ethers.keccak256(ethers.toUtf8Bytes(content))`. This is the tick-49 M2 anti-pattern class (test recomputes via the same primitive the impl uses). | **Defensible — neutralized by test 2.** On its own, test 1 is self-pinning: any bug in `ethers.keccak256` would affect both sides equally. But test 2 (L31-39) pins the **literal hex** `0xc5d246…85a470` against `hashContent("")`. That literal is independent of any ethers call and matches the canonical empty-bytes keccak256 (verified above). So if `ethers.keccak256` ever silently swaps algorithm or encoding, test 2 catches it via the hardcoded literal even though test 1 wouldn't. Test 1 then serves a different purpose: pinning the **shape contract** ("hashContent is keccak256(utf8(s))") which is what callers expect for cross-comparison with Solidity's `keccak256(bytes(...))`. The combination (algorithm-pinning literal + shape-pinning composition test) is sound. Flag-worthy if standalone, but the literal in test 2 rescues it. No finding. |
| 4 | **`assert/strict` semantics** — `assert.equal` uses `Object.is`. | **Correct for strings.** All `assert.equal` calls in this file compare strings (hash hex, content strings) or numbers (`byteLength`, `size`). `Object.is` is reference-equal for objects but value-equal for primitives — exactly what's wanted here. No coercion smell. No finding. |
| 5 | **Case-insensitivity tests genuinely exercise the normalize path?** L88 `"0x" + hash.slice(2).toUpperCase()` constructs an uppercased hex from the lowercase ethers output. | **Genuinely exercises the path.** `ethers.keccak256` returns a lowercase 0x-hex (verified via the empty-string literal in test 2 — lowercase). The test uppercases everything after `0x` (the `0x` prefix is non-alpha so toUpperCase is a no-op on it; only the hex digits change). The store's internal map is keyed by the lowercase `hash` returned from `put`. If `get`'s `this.normalize(hash)` (L52) were bypassed (e.g. `return this.byHash.get(hash)` directly), the uppercased lookup would miss and return `undefined` — failing the assertion at L89. Same for `verify` at L66, exercised by L120-126. Both genuinely test the normalize path. No finding. |
| 6 | **Comment quality** — any new comments that lie, restate, or add no information? | **Load-bearing throughout.** Header L1-12 explains *why* the test exists (SPEC citations + the R4 hard-invariant rationale) — not restating code. L25 ("Independently recomputed via ethers — pins the SPEC-0001 R3 wire format") — accurate per finding #3, identifies the wire-format-pinning purpose. L32-34 ("0xc5d2… is the canonical keccak256 of the empty byte string — pinned here so a future change to the ethers import or encoder would fail loud") — accurate per finding #2, explains the alert-on-drift purpose. L75 ("A different hash returns undefined") — restate-ish but immediately documents what the assertion is verifying, given the indirect setup. L86-87 ("hashContent returns the lowercase ethers form; uppercase the hex digits and confirm the store still finds it") — explains the deliberate uppercase construction, load-bearing. No comments lie. No finding. |
| 7 | **PHI / R1 invariant** — do test fixtures contain real-looking PHI? Specifically `"Severe plaque psoriasis; PASI 14"` (L26). | **Synthetic clinical phrasing, no PHI.** "Severe plaque psoriasis" is a clinical condition descriptor — not patient-identifying. "PASI 14" is the Psoriasis Area and Severity Index score (PASI is a public clinical scale, max 72) — not patient-identifying. No name, no DOB, no MRN, no SSN, no address, no phone, no email, no provider name, no specific date. Other fixtures: `"synthetic clinical justification — no PHI"` (L56, self-documenting), `"the note that lives off-chain"`, `"original content"`, `"tampered content"`, etc. — all generic strings. The PASI-14 example is the same shape as the `partd-approvable` scenario fixture which has already passed PHI review (tick 8). Clean. No finding. |
| 8 | **Coverage impact** — was the P2 goal met? | **100% line / 100% branch / 100% function on `content.ts`.** Independently re-ran `npx tsx --test --experimental-test-coverage src/content/content.test.ts` → reports `content.ts | 100.00 | 100.00 | 100.00 \| (none uncovered)`. All 8 exported surfaces exercised: `hashContent` (4 tests), `ContentStore.put` (2), `.get` (2), `.has` (1), `.size` (1 — exercised through `.has` and `.put` idempotency tests), `.verify` (3), `verifyContent` (3). Private `normalize` exercised transitively via case-insensitive `.get`/`.verify` tests. P2 closed cleanly. |
| 9 | **Coverage.md update needed?** P2 was queued at coverage.md L96; should it be marked closed in the same file? | **Defensible to defer this tick, but flagged as a soft follow-up.** Tick 57's pattern was to author the P-queue but defer per-item closure annotations to the closing tick. This tick (58) closes P2 with the same shape of work tick 57 did for P7 — but tick 57 didn't go back and edit P7 in the queue either; instead the closure is recorded in the strict-review-findings. Consistency favors deferring the coverage.md edit until either (a) all P1-P7 close and the queue is collapsed into a summary, or (b) a dedicated bookkeeping tick. **Soft observation, not a finding.** A finding would require the queue's text to be misleading after this tick — it isn't, since P2 still describes the *pre-tick* state and the strict-review record carries the closure. |

### New findings

**None.** Every scrutiny item resolved with in-source or in-run evidence.

### Empirical re-verification

- `npx tsx --test "src/**/*.test.ts"` → **106/106 pass** (matches the +15 claim over tick-57's 91/91 baseline).
- `npx tsx --test --experimental-test-coverage src/content/content.test.ts` → `content.ts` reports **100.00% line / 100.00% branch / 100.00% function**, closing the P2 gap.
- `node -e "const {ethers}=require('ethers'); console.log(ethers.keccak256(ethers.toUtf8Bytes('')))"` → `0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470`, byte-for-byte match with the pinned literal at content.test.ts L37.
- Spec line-numbered cross-check: R3 at `docs/specs/0001-mvp0-coverage-negotiation.md` L49, R4 at L50, T1 at L208 — all three cited correctly in the test header.
- Hardhat 30/30, harness 37/37, root + web `tsc` clean — carried forward; no contracts, no web/UI, no shared TS surface touched this tick.

### Verdict: **PASS**

The new test file closes P2 with the right contract pins: an independent literal for the empty-bytes keccak256 (catches algorithm/encoder drift), a shape-pinning composition test (catches deviation from `keccak256(utf8(s))`), determinism + cross-input distinguishability for `hashContent`, full idempotency + round-trip + miss coverage for `ContentStore`, and genuine exercise of the case-normalize path on both `get` and `verify`. The test 1 self-pinning concern is neutralized by test 2's independent literal — the combination pins both the algorithm and the composition. PHI invariant is clean; all clinical fixtures are non-identifying public clinical vocabulary (PASI score, condition descriptors). 100% line/branch/function coverage on `content.ts` confirmed empirically. R3/R4/T1 citations map to the right spec lines. Fit to commit.

## Tick 59 strict-review

**Date:** 2026-05-30
**Scope:** 1 new file:
- `src/integrations/cds-hooks/mapper.test.ts` (276 LOC, 17 tests) — closes the P3 gap from tick 57's coverage queue (`src/integrations/cds-hooks/mapper.ts` was at 0% line / 0% branch / 0% function before this tick).

R-citations the unit claims to satisfy: **SPEC-0002 R7** (CDS-Hooks integration SEAM, mocked) + **SPEC-0004 R1** (Synthetic notes only / no real PHI) + **SPEC-0001 R2 / R6a** (quantity drives the deterministic cap; must be > 0). Verified inline:
- SPEC-0002 R7 at `docs/specs/0002-demo-experience-and-integration-seam.md:51` defines the CDS-Hooks `order-sign` seam.
- SPEC-0004 R1 at `docs/specs/0004-data-and-evidence-model.md:29` defines "Synthetic notes only. Zero real PHI in the demo bundle".
- SPEC-0001 R2 at `docs/specs/0001-mvp0-coverage-negotiation.md:48` defines the quantity field as the NDC-pinned cap driver; R6a at L53 defines the deterministic-cap rule using `quantity` as multiplier.

All three citations map correctly. The header comment at mapper.test.ts L4-7 cites R7 + R1 + R2/R6a; the per-test cite for the zero-quantity guard at L215 names SPEC-0001 R2/R6a.

### Scrutiny checklist (per tick-59 prompt)

| # | Item | Finding |
|---|------|---------|
| 1 | **PHI guard strength** — happy-path asserts `!justification.includes("synthetic-pt-001")` + `!justification.includes("Patient/")`. Could a future impl drift to a different PHI marker these miss? | **Strong but not exhaustive — defensible for this seam.** The two markers cover the two structural PHI vectors in `OrderSignContext`: (a) the bare patientId `"synthetic-pt-001"` (L60 of test, mirrored to the impl as `req.context.patientId`), and (b) the `subject.reference` form `"Patient/synthetic-001"` (L38 of `makeOrder`, mirrored to the impl as `order.subject?.reference`). The fixture happy-path uses the full `"Patient/synthetic-pt-001"` form (fixture.ts L57) — so both markers are present in the input and both are asserted absent in the output. A theoretical drift the test wouldn't catch: an impl that emits `userId` (Practitioner ref), `encounterId`, or copies `subject.display` ("Synthetic Patient (demo)"). However, the mapper's input surface only carries `patientId` + `subject` as the PHI carriers per the FHIR R4 `MedicationRequest` shape it consumes — Practitioner/Encounter ids are not PHI under HIPAA Safe Harbor (provider identifiers) and `subject.display` would still be caught by the `"Patient/"` prefix check on most realistic refs. Test at L156 (`justification is single-spaced + no PHI`) and the diagnosis-presence test at L135 redundantly assert both markers. The PHI guard is the strongest practical pin given the input shape; a stronger pin would be a positive whitelist on justification tokens, but the current negative-pin pair is appropriate for a regex-friendly assertion. **No finding.** A soft observation: a future drift to `subject.display` exfiltration (e.g. emitting a patient name string that doesn't start with "Patient/") would slip through — but the current input shape doesn't carry a realistic name, so the risk is theoretical. |
| 2 | **Spec citations** — SPEC-0002 R7 + SPEC-0004 R1 + SPEC-0001 R2/R6a. | **All three verify.** Line-numbered cross-check above. R7 = CDS-Hooks seam at SPEC-0002:51. R1 = synthetic / no-PHI at SPEC-0004:29. R2/R6a = quantity > 0 cap driver at SPEC-0001:48/53. The L215 in-test cite (`SPEC-0001 R2/R6a`) for the zero-quantity guard is the right pair: R2 establishes quantity as the cap basis and R6a establishes that `coveredAmount = min(requested, perUnit × quantity)` collapses to zero when quantity=0, which is why the guard exists. **No finding.** |
| 3 | **Fixture builder helpers** (`makeOrder`, `makeRequest`) — minimal but valid? Could a FHIR R5 migration silently break the test before the impl? | **Minimal-valid and shape-coupled.** `makeOrder` (L21-45) sets only the fields the mapper reads: `resourceType`, `status`, `intent`, `medicationCodeableConcept`, `subject`, `dispenseRequest` — all FHIR R4 required-or-typical. Required-by-types-but-not-by-mapper fields (`id`) are correctly omitted. `makeRequest` (L48-64) builds a CDS Hooks 2.0 envelope with the minimum CDS Hooks 2.0 requires (`hook`, `hookInstance`, `context`). The fixed hookInstance UUID is fine for unit tests (no determinism dependency). On FHIR R5: R5 renamed `medicationCodeableConcept` → `medication` (now a `CodeableReference`) and tightened `dispenseRequest.quantity` to require a positive value at the schema level. If the team migrates the type interfaces to R5, the mapper's `order.medicationCodeableConcept` access would type-error against the new shape — surfacing the break at the impl, not silently in the test. The test would fail-loud as the type rename propagates. **No finding** — the helpers are correctly coupled to the impl's read surface, not to incidental FHIR R5 details. |
| 4 | **NaN test interpretation** — `Number.NaN` goes through which branch? | **Correct.** `toBigInt` at mapper.ts L88-96 checks `Number.isNaN(value)` on L89 BEFORE the `!Number.isFinite(value)` check on L92. NaN matches the first guard and throws `missing ${field}` — which is `missing dispenseRequest.quantity.value` per the call at L118. The test at L228-239 expects exactly that regex. The "NaN branch" comment in the test name (L228) is precise: NaN is technically also `!Number.isFinite` (NaN is non-finite), but the order of checks routes it to the "missing" path. The test correctly pins the priority. **No finding.** A subtle point worth noting: this is intentional impl design — NaN is treated as a missing/malformed value rather than an out-of-range value, which is the right call for FHIR JSON parsing (a NaN in JSON would only arise from a malformed numeric encoding). |
| 5 | **Negative + Infinity tests** — both expect `/must be a finite, non-negative number/`. | **Both correctly hit `!isFinite \|\| value < 0`.** Infinity (`Number.POSITIVE_INFINITY`) at L257: passes the L89 check (not undefined/null/NaN), then on L92 `!Number.isFinite(Infinity)` is `true` → throws the "finite, non-negative" message. Negative (`-1`) at L244: passes L89 (not undefined/null/NaN), then on L92 `!Number.isFinite(-1)` is `false` but `value < 0` is `true` → same throw. Both regexes match. **No finding.** |
| 6 | **Type-cast in error-path test** — `{ resource: { resourceType: "Patient" as never } }` at L184. Honest about constructing an invalid FHIR shape? Could `as never` mask a future API surface change? | **Honest cast, narrow exposure.** The `as never` cast is required because `FhirResource` is the union `FhirMedicationRequest | { readonly resourceType: string; ... }` — the second branch accepts any non-MedicationRequest resourceType string, but `FhirBundleEntry<TResource>` defaults `TResource` to `FhirResource` and the inferred discriminator on `resource` makes TS narrow to `FhirMedicationRequest` when `resourceType: "MedicationRequest"` is asserted. The cast forces a Patient-shaped resource into the entry slot. Looking at `types.ts` L96-106: `FhirResource = FhirMedicationRequest \| { readonly resourceType: string; ... }` — so `{ resourceType: "Patient" }` actually satisfies the second branch of the union without needing `as never`. The cast is therefore over-aggressive but not dishonest — a cleaner test would write `{ resource: { resourceType: "Patient" } as FhirResource }` or simply `as { readonly resourceType: string }`. Future API surface risk: if `FhirResource` is narrowed to a discriminated union of known resources (e.g. `FhirMedicationRequest \| FhirPatient \| ...`), the `as never` would silently continue to compile while the saner cast would error and force a test update. **Soft observation, not a finding** — the test exercises the correct impl branch (the `find` returns undefined → second guard fires) and the impl correctness verification is intact. Suggestion for a follow-up tick: replace `as never` with `as FhirResource` so the cast widens to the intended union and surfaces future surface-changes. |
| 7 | **Idempotency / determinism** — fixtures pure (no Date.now, no Math.random)? | **Pure.** All fixture builders construct from literal values. `makeRequest` uses a fixed all-zero UUID for `hookInstance` (L57). `SAMPLE_ORDER_SIGN_REQUEST` (fixture.ts) is a module-level const literal. No `Date.now`, `Math.random`, environment reads, or filesystem reads anywhere in the test or its imports. Tests are reproducible. **No finding.** |
| 8 | **Coverage delta — close P3 in coverage.md?** Run coverage to confirm `mapper.ts` is at/near 100%. | **`mapper.ts`: 100.00% line / 95.65% branch / 100.00% function** — verified by `node --import tsx --test --experimental-test-coverage --test-coverage-include='src/integrations/cds-hooks/mapper.ts' src/integrations/cds-hooks/mapper.test.ts`. The two uncovered branches (LCOV `BRDA:26,20,0,0` and `BRDA:28,23,0,0`) correspond to defensive null-guards inside `conceptText`/`drugCodeOf` — specifically the `concept === undefined` branch in `conceptText` (the impl L73 `if (!concept) return undefined;` — never reached because all callers pass a defined CodeableConcept) and the `coding?.code ?? undefined` final fallback when both display AND code are absent on the first coding (unreachable in any FHIR-valid input where coding is present at all). Both are pure defensive guards on impossible-in-FHIR-valid-input paths; 95.65% branch with two unreachable defensive branches is appropriate for a pure mapper. P3's queued threshold was "line ≥ 85%" — exceeded by 15pp. **P3 effectively closed.** Per the tick-58 precedent (defer per-item closure annotations in `coverage.md` to a bookkeeping tick), the queue text remains accurate for the pre-tick state; the closure is recorded here. **No finding.** |
| 9 | **Comment quality + R-citations** — any new comments that lie, restate code, or misquote R-numbers? | **Load-bearing throughout.** Header L1-8 cites R7/R1 (correctly) and pins the rationale. L13-18 in the type-imports is just module structure. The per-section dividers at L66-68 and L168-170 are wayfinding only. Inline comments: L72 ("The fixture's drug text starts with 'Adalimumab' — pin the prefix") — accurate against fixture.ts L54. L74 ("quantity + daysSupply are positive bigints") — accurate. L79 ("SPEC-0004 R1 invariant: no PHI passes through to the justification") — accurate citation. L113 ("Display fallback path — text is empty string (whitespace-only)") — accurate per `conceptText` L74 (`concept.text.trim().length > 0`). L125 ("Code-only fallback path") — accurate. L143 ("Diagnosis SHOULD appear in the justification when present") — accurate per mapper.ts L132. L149 ("No reasonCode provided") — accurate (the `makeOrder({})` call passes no `reasonCode`). L159 ("No double spaces") — accurate per mapper.ts L135 `.replace(/\s+/g, " ")`. L182 ("Stash a non-MedicationRequest resource so entries.length > 0 but find() returns undefined") — accurate intent for the second-guard test. L215 in-test ("SPEC-0001 R2/R6a") — citation pair verified above. L228 ("toBigInt NaN branch") — accurate per finding #4. **No lying, no restating, no misquotes.** **No finding.** |
| 10 | **`assert.throws` regex precision** — specific enough vs not brittle to wording tweaks? | **Right tradeoff.** Each regex pins the load-bearing tokens from the impl message and escapes nothing extraneous. `/draftOrders bundle is empty/` (L176) matches the impl's `"draftOrders bundle is empty — no order to map."` (mapper.ts L57) — pinned on the noun phrase that signals the failure mode. `/no MedicationRequest resource/` (L188) matches the impl's `"draftOrders contains no MedicationRequest resource."` (L65) — pinned on the missing-resource phrase. `/no medicationCodeableConcept name\/code/` (L198) matches L113. `/missing dispenseRequest\.quantity\.value/` (L211, L237) matches the L118 call site. `/dispense quantity must be > 0/` (L224) matches L120 — pins the SPEC-0001 R2/R6a guard. `/must be a finite, non-negative number/` (L250, L263) matches L93. All escape `.` and `/` correctly. Wording tweaks like changing "must be" → "should be" or "draftOrders" → "draft orders" would break the regexes — but those are semantically meaningful changes that should fail the test. Punctuation-only tweaks (em-dash, trailing period) don't affect any of the pinned tokens. **No finding.** |

### New findings

**None.** Every scrutiny item resolved with in-source or in-run evidence. The `as never` cast at L184 is flagged as a **soft observation** (not a finding) — the cast is over-aggressive but not dishonest, and the impl branch it exercises is correctly verified. Suggested follow-up: replace with `as FhirResource` so the cast widens to the intended union.

### Empirical re-verification

- `npx tsx --test 'src/**/*.test.ts'` → **123/123 pass** (matches the +17 claim over tick-58's 106/106 baseline).
- `node --import tsx --test --experimental-test-coverage --test-coverage-include='src/integrations/cds-hooks/mapper.ts' src/integrations/cds-hooks/mapper.test.ts` → `mapper.ts` reports **100.00% line / 95.65% branch / 100.00% function**, closing the P3 gap (queued threshold was line ≥ 85%; exceeded by 15pp). The two uncovered branches are defensive null-guards on FHIR-valid-input-unreachable paths inside `conceptText` and `drugCodeOf`.
- Spec line-numbered cross-check: SPEC-0002 R7 at L51, SPEC-0004 R1 at L29, SPEC-0001 R2 at L48, R6a at L53 — all four cited correctly in the test header / inline.
- NaN-branch trace: `toBigInt(NaN, ...)` → L89 `Number.isNaN(NaN)` true → throws `"missing ${field}"` (not the `"finite, non-negative"` message) — test at L228-239 correctly anchors on the "missing" regex.
- Infinity / negative trace: both bypass the L89 NaN/null/undefined guard and hit the L92 `!Number.isFinite(value) \|\| value < 0` guard — test at L241-265 correctly anchors on the "finite, non-negative" regex.
- Hardhat 30/30, harness 37/37, root + web `tsc` clean — carried forward; no contracts, no web/UI, no shared TS surface touched this tick.

### Verdict: **PASS**

The new test file closes P3 with the right contract pins: a happy-path against the SAMPLE fixture that asserts both PHI markers (`patientId` form and `subject.reference` form) absent from the justification; the RxNorm-preferred / NDC-fallback drugCode selection; the text → display → code conceptText fallback chain; the reasonCode[0] → diagnosis propagation (present-and-absent); the EHR provenance marker and single-spacing of the justification; and every guard-error path with regex-precision anchors that pin load-bearing tokens without over-fitting. The NaN ordering interpretation is correct (NaN routes to "missing" by the L89-first check, not "finite, non-negative"). Negative + Infinity both correctly hit the L92 guard. Fixtures are pure (no Date.now, no Math.random). Spec citations (SPEC-0002 R7, SPEC-0004 R1, SPEC-0001 R2/R6a) all verify against the canonical R-numbered lines. Coverage: 100% line / 95.65% branch / 100% function on `mapper.ts` — the two uncovered branches are defensive null-guards on unreachable FHIR-valid paths. The `as never` cast at L184 is a soft observation worth a future cleanup but doesn't compromise the test's correctness. Fit to commit.

## Tick 60 strict-review

**Date:** 2026-05-30
**Scope:** 1 new untracked file vs `7c5ad84`:
- `src/integrations/cds-hooks/fixture.test.ts` (153 LOC, 12 tests) — closes coverage gap **P4** from tick-58's queue (`src/integrations/cds-hooks/fixture.ts` was at 0% line / 0% branch / 0% function before this tick; queued threshold `line ≥ 85%`).

R-citations the unit claims to satisfy: **SPEC-0004 R1** (Synthetic notes only / zero real PHI in the demo bundle) + **SPEC-0001 R2** (request carries `quantity` — NDC-pinned cap driver) + **SPEC-0001 R6a** (`coveredAmount = min(requested, perUnit × quantity)` — quantity must be > 0 for a non-zero cap). Verified inline:
- SPEC-0004 R1 at `docs/specs/0004-data-and-evidence-model.md:29` — "Synthetic notes only. Zero real PHI in the demo bundle, including in fixtures, snapshots, or test data."
- SPEC-0001 R2 at `docs/specs/0001-mvp0-coverage-negotiation.md:48` — quantity is the NDC-pinned cap driver.
- SPEC-0001 R6a at `docs/specs/0001-mvp0-coverage-negotiation.md:53` — quantity is the multiplier in the deterministic cap.

The header comment at fixture.test.ts L4-10 cites R1 + R2/R6a. The per-test comments at L46 ("SPEC-0004 R1: NO PHI in the fixture") and L130-132 ("SPEC-0001 R2/R6a") name the right R-numbers.

### Scrutiny checklist (per tick-60 prompt)

| # | Item | Finding |
|---|------|---------|
| 1 | **PHI patterns adequacy** — four regex patterns (SSN/DOB/phone/email). Are they the right ones? Could PHI sneak through (SSN no-dashes, international phone, plus-form email)? | **Adequate-and-honest for a loud-signature scanner.** The four patterns cover the canonical en-US forms: SSN `\b\d{3}-\d{2}-\d{4}\b`, DOB `"\d{4}-\d{2}-\d{2}"`, phone in both parenthesized `\(\d{3}\)\s?\d{3}-\d{4}` and dash-separated `\b\d{3}-\d{3}-\d{4}\b` forms, and a rough email pattern. Known gaps: (a) a 9-digit-run SSN with no dashes (`123456789`) would not match — but `00074-3799-02` (NDC) is dash-separated and shaped wrong for SSN-no-dashes anyway; (b) international phones with country codes (`+1...`) would not match; (c) DOBs without surrounding quotes would not match; (d) emails using uncommon TLDs are caught by the `{2,}` quantifier so this is not actually a gap. The test is positioned as a **loud signature scanner** for "real-shape" PHI, not an exhaustive PHI detector — which matches the SPEC-0004 R1 intent ("a future copy of real patient data fails this test loud"). A real-form SSN/phone/DOB/email in any future fixture edit will trip exactly the markers a casual paste would carry. **No finding.** Soft observation: a positive-form whitelist (every string must match `/synthetic\|adalimumab\|Humira\|L40\|ICD-10\|RxNorm\|.../`) would be stronger but unwieldy; the current negative-pattern scanner + the synthetic-marker assertion in the prior test (L49-62) together form a defense-in-depth pair that is fit for a loud-CI tripwire. |
| 2 | **DOB false-positive risk** — `/"\d{4}-\d{2}-\d{2}"/` would match any 4-2-2 quoted segment. Could a UUID component, ISO timestamp, or future field trip it? | **Empirically clean against the current fixture; future-trip is by-design.** The hookInstance UUID is `"3f1a8c2e-9b7d-4e6a-8c1f-2d5b6a7c8e90"` — UUIDs split 8-4-4-4-12, never 4-2-2, and contain non-digit hex (the first segment `3f1a8c2e` has letters `f`/`a`/`c`/`e`). The fixture has zero other quoted strings of digit-only `dddd-dd-dd` shape (verified by serializing the fixture and running the regex: returns `null`). If a future fixture edit adds an ISO date-only string like `"2026-05-30"` for a fictional clinical date, it would correctly trip this test — that's the intended "any date-shape in a PHI surface requires explicit review" loudness. The comment at L69-70 anticipates this: "Date of birth yyyy-mm-dd that isn't the access-date in a comment (not expected in this fixture — verify clean)." **No finding.** |
| 3 | **UUIDv4 regex strictness** — `/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i`. Strict enough to catch a real-UUID copy-paste? | **Canonical-v4-strict.** The `4` in the third segment pins version=4 (a v1/v3/v5 UUID would fail). The `[89ab]` in the fourth segment pins the RFC 4122 variant bits. A pasted real UUIDv4 would match — which is correct, because UUIDv4 is intentionally cryptographically random and a "real" v4 is shape-indistinguishable from a synthetic v4 (this is the point of v4). The test pins **structure**, not provenance — which is the right semantic for a hookInstance, where uniqueness rather than synthetic-marker is the property under test. The synthetic-marker discipline lives on the other identifiers (Practitioner/Patient/Encounter ids) per the L49-62 test, which is correct. **No finding.** |
| 4 | **Synthetic-id assertion strength** — `/synthetic/i` on every identifier. Could a real Patient/MRN-1234567 slip through if `synthetic-` prefix were removed? | **Strong — exactly the loudness intended.** If a future edit changed `Patient/synthetic-pt-001` to `Patient/MRN-1234567`, the `/synthetic/i` regex fails on `ctx.patientId` (L54). The same for userId (L53), encounterId (L55), `subject.reference` (L60), and `subject.display` (L61). Five independent assertions cover the five identifier surfaces in the fixture. A drift that removed only the `synthetic-` prefix would fail loud on every one of the five. The `i` flag is a small kindness (a future "Synthetic" capitalization wouldn't trip a case-sensitive variant) and doesn't weaken the marker. **No finding.** |
| 5 | **R-citations** — SPEC-0004 R1 + SPEC-0001 R2/R6a verified in the file? | **All three verify.** Header L4-10 cites R1 + R2/R6a with prose explanation. Section divider at L46-47 names "SPEC-0004 R1: NO PHI in the fixture". Section divider at L130-132 names "SPEC-0001 R2/R6a: dispense quantity drives the deterministic cap" + per-test comment at L134 ("SPEC-0001 R2/R6a"). The R-numbers map to: R1 at SPEC-0004:29, R2 at SPEC-0001:48, R6a at SPEC-0001:53 — cross-checked via grep. **No finding.** |
| 6 | **`Extract` type cast in `firstOrder()`** — honest about the runtime narrowing performed by `assert.equal(resource.resourceType, "MedicationRequest")`? | **Honest cast — node:assert/strict doesn't narrow TS types.** `assert.equal` from `node:assert/strict` is not a TypeScript type predicate (its signature is `(actual, expected) => void`, not `(actual) => asserts actual is T`), so the resource binding still has the union type `FhirMedicationRequest \| { resourceType: string; ... }` after the call at L88. The `Extract<typeof resource, { resourceType: "MedicationRequest" }>` cast at L89-92 then explicitly widens-then-narrows to the MedicationRequest branch. Because the cast immediately follows a runtime check that proves the same fact, the cast is **load-bearing-and-honest** — it bridges a real TS-narrowing gap rather than papering over an unverified assumption. The cleaner-but-equivalent alternative would be a user-defined `assertIsMedicationRequest` predicate; the current pattern is the standard node-assert idiom and is broadly understood. **No finding.** |
| 7 | **Comment quality** — any that lie, restate, or add no information? | **All load-bearing.** Header L1-10: pins the spec citations + the test's contract intent (the "loud-fail on PHI paste" property). L29-30: explains the UUIDv4 version/variant nibble check AND pins the fixture's specific instance value (load-bearing — a future fixture-instance change would surface here). L50-51: states the SPEC-0004 R1 contract (every id must contain "synthetic" so a real-data paste fails loud). L57-58: explains why the subject reference + display also must be synthetic. L65: ("Serialize the whole fixture and scan for PHI-resembling tokens") explains the regex-scan strategy. L67-76: each regex line names the PHI class. L69-70: anticipates the not-expected-in-this-fixture case (load-bearing future-edit warning). L74 ("US phone xxx-xxx-xxxx"): correctly disambiguates from the parenthesized form on L73. L76 ("Email (rough)"): honest about the regex precision. L117 ("The demo's adalimumab RXCUI is 1366724"): pins the cross-reference to demo-data/sample-case.md. L131-132: SPEC-0001 R2/R6a section header. No lying, no restating, no padding. **No finding.** |
| 8 | **hookInstance UUIDv4 validity** — is `3f1a8c2e-9b7d-4e6a-8c1f-2d5b6a7c8e90` a valid v4? | **Valid v4.** Segment-by-segment: `3f1a8c2e` (8 hex), `9b7d` (4 hex), `4e6a` (4 hex — version nibble `4` ✓), `8c1f` (4 hex — variant nibble `8` ∈ `[89ab]` ✓), `2d5b6a7c8e90` (12 hex). The regex at L33 accepts it. Empirically the test passes (`ok 2 - SAMPLE_ORDER_SIGN_REQUEST.hookInstance is a UUIDv4-shaped string`). **No finding.** |
| 9 | **`!` non-null assertion silent-mask risk** — could a future fixture edit that legitimately removes an optional field silently mask a failure? | **Loud-fail, not silent-mask.** Every `!` in the test bodies (e.g. `ctx.encounterId!` at L55, `order.subject!.reference!` at L60, `dispenseRequest!.quantity!` at L136, `expectedSupplyDuration!` at L142/L150, `med.coding ?? []` at L112 — fallback, not `!`, but acts the same) corresponds to a field that's `optional` in the FHIR types but **present-and-required-by-contract** in the fixture. If a future edit removed `encounterId` from the fixture, `ctx.encounterId!` would evaluate to `undefined`, and `/synthetic/i.test(undefined)` (via `assert.match`) throws `TypeError: Argument must be a string` — surfaces as a loud test failure (not a silent skip). Similarly for `order.subject!.reference!` — removing `subject` would TypeError on `.reference` access. The contract: every `!` in this test is a "this field MUST be present in the fixture" pin. **No silent-mask risk; no finding.** |
| 10 | **Coverage delta — close P4 in coverage.md?** Run coverage to confirm `fixture.ts` is near 100%. | **`fixture.ts`: 100.00% line / 100.00% branch / 100.00% function** — verified by `node --import tsx --test --experimental-test-coverage --test-coverage-include='src/integrations/cds-hooks/fixture.ts' src/integrations/cds-hooks/fixture.test.ts`. The fixture is a module-level const literal with zero functions and zero conditionals; "100% line" is trivially achieved by importing the module + dereferencing every field path the tests visit. P4's queued threshold was `line ≥ 85%` — exceeded by 15pp. The two zero-cells in coverage.md L60 (`0 \| 0 \| 0 \| GAP`) for `fixture.ts` are now closeable. Per the tick-58/59 precedent (defer per-item closure annotations to a bookkeeping tick), the queue text on L97 remains accurate for the pre-tick state; the closure is recorded here. **No finding.** |

### New findings

**None.** Every scrutiny item resolved with in-source, in-spec, or in-run evidence. The PHI scanner is positioned as a loud-signature tripwire (not an exhaustive PHI detector) and is sized correctly for that role; the DOB false-positive risk is empirically zero against the current fixture and is by-design loud against any future quoted-date addition; the synthetic-marker discipline is enforced across all five identifier surfaces with case-insensitive matching; the UUIDv4 regex pins canonical version+variant nibbles; the `Extract` type cast is the standard node-assert idiom and honest about the TS-narrowing gap; comments are load-bearing; the `!` non-null assertions are loud-fail tripwires for "field MUST be present in fixture" contracts; the fixture's hookInstance is a valid UUIDv4 by RFC 4122 inspection.

### Empirical re-verification

- `npx tsx --test src/integrations/cds-hooks/fixture.test.ts` → **12/12 pass**.
- `npx tsx --test 'src/**/*.test.ts'` → **135/135 pass** (matches the +12 claim over tick-59's 123/123 baseline).
- `node --import tsx --test --experimental-test-coverage --test-coverage-include='src/integrations/cds-hooks/fixture.ts' src/integrations/cds-hooks/fixture.test.ts` → `fixture.ts` reports **100.00% line / 100.00% branch / 100.00% function**, closing the P4 gap (queued threshold was `line ≥ 85%`; exceeded by 15pp).
- Fixture serialized + regex-scanned in-process: SSN regex `null`, DOB regex `null`, phone-dash regex `null`. Empirically confirms the PHI-scanner test would pass against the current fixture and the DOB false-positive risk against existing fields (UUIDs, NDC code, RxCUI) is zero.
- Spec line-numbered cross-check: SPEC-0004 R1 at L29 (Synthetic notes only / Zero real PHI), SPEC-0001 R2 at L48 (quantity = NDC-pinned cap driver), SPEC-0001 R6a at L53 (deterministic cap formula) — all three cited correctly in the test header / per-test comments.
- UUIDv4 manual validation: `3f1a8c2e-9b7d-4e6a-8c1f-2d5b6a7c8e90` — version nibble `4`, variant nibble `8` — RFC 4122 v4 valid.
- Hardhat / web / contract layers carried forward: no contract, no web/UI, no shared TS surface touched this tick.

### Verdict: **PASS**

The new test file closes P4 with a tight, structurally-grounded contract: the CDS Hooks envelope (`hook`/`hookInstance`/`context.*Id` prefixes); the SPEC-0004 R1 NO-PHI invariant enforced two ways (synthetic-marker discipline on every identifier + a loud-signature scanner for real-shape SSN/DOB/phone/email); the FHIR Bundle shape (single-entry collection of a draft-order MedicationRequest); the RxNorm + NDC coding pair with a pinned RxCUI for adalimumab; the ICD-10-CM L40.x reasonCode for psoriasis; and the SPEC-0001 R2/R6a positivity invariant on dispense quantity + supply duration with UCUM-coded units. The PHI scanner's known gaps (SSN-no-dashes, international phones, unquoted DOBs) are correctly scoped to a loud-tripwire role — exhaustive PHI detection isn't this test's job. The UUIDv4 regex pins canonical version+variant nibbles. The `Extract` type cast is honest about bridging the node:assert TS-narrowing gap. The `!` chains are loud-fail tripwires, not silent masks. Comments are load-bearing throughout (none restate code, none lie about citations). Coverage: 100% line / 100% branch / 100% function on `fixture.ts` — full closure of P4. Fit to commit.

## Tick 61 strict-review

**Date:** 2026-05-30 · **Branch:** `spec-4-implementation` · **HEAD vs:** `e70fede`
**Diff:** 1 new file — `src/profiles/profileRegistry.test.ts` (~140 lines, 12 tests).
**Claim:** closes coverage gap **P5** — `profiles.ts` was at 84.92% line / 75% branch with the `ProfileRegistry` class methods + constructor guards untested.

### Empirical re-verification

- `npx tsx --test src/profiles/profileRegistry.test.ts` → **12/12 pass**.
- `npx tsx --test 'src/**/*.test.ts'` → **147/147 pass** (matches the +12 claim over tick-60's 135/135 baseline).
- `node --import tsx --test --experimental-test-coverage --test-coverage-include='src/profiles/profiles.ts' src/profiles/profileRegistry.test.ts src/profiles/profiles.test.ts` → `profiles.ts` reports **100.00% line / 100.00% branch / 100.00% function** (was 84.92 / 75 / 66.67). P5 acceptance criterion was `line + branch ≥ 85%` — exceeded by 15pp on line and 25pp on branch; all three dimensions are at ceiling.

### Scrutinize-for-zero-findings checklist (per tick-61 prompt)

| # | Item | Finding |
|---|------|---------|
| 1 | **Constructor-guard coverage — regex matches the exact thrown messages.** | **Both regexes match.** Empty-list: thrown `"ProfileRegistry requires at least one profile."` (profiles.ts L74); test regex `/requires at least one profile/` matches the substring. Unknown-activeId: thrown `` `Unknown initial activeId "${wanted}".` `` (L80) → for `wanted="ghost"`, produces literal `Unknown initial activeId "ghost".`; test regex `/Unknown initial activeId "ghost"/` matches the double-quoted literal exactly (double-quotes are not regex metacharacters). Both guards are pinned to their exact wording, not loosely on a keyword — a future error-message rewording would correctly trip the assertion. **No finding.** |
| 2 | **`listProfiles` mutation-isolation test — does it actually detect an internal-array leak, or accidentally pass if the impl returned the internal array?** | **Genuinely detects a leak.** Impl is `return [...this.profiles.values()]` (profiles.ts L91-93) — a fresh array per call. The test does `a.push(extra); assert.equal(reg.listProfiles().length, DEFAULT_PROFILES.length)`. Hypothetical leaky impls evaluated: (a) `return Array.from(this.profiles.values())` cached as a field — second call returns the same mutated array; assertion `length === 2` would FAIL because mutated length is 3. (b) `return this.profiles.values()` — `.push` would TypeError because MapIterator has no `.push`, surfacing as a loud test failure (still fails-not-passes). (c) Direct internal `private profiles: Profile[]` returned bare — `a === reg["profiles"]`; the push mutates the internal; second call returns the same array of length 3; assertion fails. The "push errored" hypothetical only applies if the returned object is iterable-but-not-Array (case b), which fails the test loud rather than masking. The test is a valid "fresh-array contract" pin. **Soft note:** an even tighter form would also assert `reg.listProfiles() !== a` (identity-distinct on each call), but that's belt-and-suspenders; the length check covers the load-bearing contract. **No finding.** |
| 3 | **Replace semantic — `addProfile` with existing id replaces, not duplicates.** | **Correct.** Impl is `this.profiles.set(profile.id, profile)` (L119); JS `Map.set` on an existing key replaces value and leaves size unchanged. Test creates a `replacement` with `id: "provider"` (same as DEFAULT_PROFILES[0]), calls `addProfile(replacement)`, then asserts (a) `getProfile("provider")?.label === "Replaced Provider"` (proves the value was overwritten, not the registration order swapped) and (b) `listProfiles().length === DEFAULT_PROFILES.length` (proves size didn't grow). Both assertions together pin "replace, not duplicate and not no-op." **No finding.** |
| 4 | **Seed string fix — `createWallet({ seed: "test-seed-1" })` is a string per `wallet.ts:99`; no stale numeric-seed test remains.** | **Clean.** `CreateWalletOptions.seed: string \| undefined` declared at wallet.ts L99. The single `createWallet({...})` call in this test file passes `seed: "test-seed-1"` (string). Grep across `src/` for `seed:\s*[0-9]` returns zero hits — no numeric seed lurks elsewhere. The wallet factory is exercised via `wallet()` helper at L15-17 with deterministic input each call. **No finding.** |
| 5 | **`assert.equal` identity on `getWallet`** | **Correct semantic.** `node:assert/strict`'s `assert.equal` aliases `assert.strictEqual`, which uses `Object.is`. For two object references, `Object.is(a, b)` is reference identity (modulo NaN/±0 edge cases that don't apply to non-primitive wallet instances). The test `assert.equal(reg.getWallet(), w)` correctly asserts the registry returned the *same* wallet object passed at construction, not a structurally-equal clone. **No finding.** |
| 6 | **R-citations — header cites SPEC-0001 R11/R12.** | **R12 is exact; R11 is a stretch but defensible.** SPEC-0001 R12 at L63 ("Under single-wallet the two addresses are equal; parties are then distinguished by the app-level `partyId` argument…The UI shows active **wallet + profile + mode**; profile switching sets identity.") is the canonical citation for `ProfileRegistry` behavior — profile binds to a `partyId`, the active id flips, the wallet is shared. The test pins exactly that. R11 at L62 ("Each request registers a **provider address** and an **insurer address**. Every party action is gated `msg.sender ∈ {providerAddr, insurerAddr}`…") is **contract-side msg.sender gating** — `ProfileRegistry` itself does not enforce it; it provides the `partyId` that the contract client uses to construct requests. The profiles.ts module header (L1-2) cites "R12/R13", not R11. The test file cites "R11/R12". The mismatch is between R13 (self-claim, naturally supported because two profiles can share a wallet — implicitly covered by the "both default profiles share one wallet" structure) and R11 (wallet-auth gating, contract-side). Calling out **soft observation:** the more accurate citation would be R12 + R13 (matching profiles.ts itself), or R12 alone — R11 is contract-side and not directly tested here. This is a citation-precision nit, not a substantive finding: the test still pins the right behavior; it just attaches a slightly-too-broad R-tag. Per tick-60 precedent (citation pedantry that doesn't change what's tested isn't a substantive finding), surfacing as a **soft observation, not a finding**. |
| 7 | **Comment quality — any that lie or restate the code?** | **All load-bearing or section-marker.** Header (L1-8) cites the spec contract and disambiguates from the existing `profiles.test.ts` (load-bearing — explains why this file exists alongside the other). L78: "Provider is the default-first profile with partyId 1n." — pins the magic `1n` to the DEFAULT_PROFILES schema (load-bearing — a future schema change would surface here). L86-87: "The registry's internal state is unaffected." — clarifies the test's intent (the assertion alone could be read as a trivial length pin). L108: "Active id is unchanged after a failed flip." — explains why the second assertion exists (load-bearing — pins the throw-is-atomic property). L133-134: "The registry size doesn't grow on a replace." — explains the second assertion's purpose. Section dividers at L26-28, L66-68, L90-92, L112-114 group tests by API surface, which aids navigation. No comments restate code, none lie about citations, none are padding. **No finding.** |
| 8 | **Test independence — each test constructs a fresh wallet + registry.** | **Fully independent.** Each of the 12 tests calls `wallet()` (which calls `createWallet`) and `new ProfileRegistry(...)` in its own closure; no module-level mutable state is shared. The module-level `extra` const (L19-24) is a `Profile` with all `readonly` fields — TypeScript-enforced immutable and the tests never mutate it (only `push` it onto a local array in test 7, which is the array-mutation test). The `DEFAULT_PROFILES` import is also `readonly Profile[]` and is never mutated. Test order is irrelevant — running any single test in isolation produces the same result as running the full file. Empirically, the test runner reports all 12 pass independently. **No finding.** |
| 9 | **Coverage delta — close P5 in `coverage.md`?** | **Yes, P5 closes.** Pre-tick: `profiles.ts` 84.92% line / 75% branch / 66.67% function (per coverage.md L23, L58). Post-tick (tool-verified above): **100% / 100% / 100%**. P5 acceptance criterion (coverage.md L98): `line + branch ≥ 85%` — exceeded on both dimensions and on function-coverage too. Per the tick-58/59/60 precedent (defer the per-item closure annotation in coverage.md to a bookkeeping tick), the queue text at L98 remains accurate for the pre-tick state; the closure is recorded here. **No finding.** |
| 10 | **Implicit additional verification — fresh-wallet's seed determinism doesn't leak across tests.** | **Safe — seed is the same string per call but each `createWallet` returns a fresh `SimulatedWallet` instance.** Two registries constructed with `createWallet({ mode: "simulated", seed: "test-seed-1" })` will have *equal-address* wallets but distinct object identities. The `getWallet` identity test (L70-74) explicitly constructs `w` once and passes it in, comparing against `reg.getWallet()` (same instance). Other tests don't compare wallet identities. The shared seed is intentional (deterministic addresses for reproducibility) and doesn't leak state because `SimulatedWallet` holds no per-test mutable state that any test reads. **No finding.** |

### Soft observations (not findings)

- **R-citation precision** (item 6): the test header cites "R11/R12" but R12/R13 would be a tighter match (matching profiles.ts itself). R11 is contract-side msg.sender auth — not enforced by `ProfileRegistry`. Cosmetic; doesn't change what's tested.
- **listProfiles freshness contract** (item 2): the test pins length-isolation but not array-identity-distinctness across calls. The cleaner-but-equivalent form would additionally assert `reg.listProfiles() !== reg.listProfiles()`. Belt-and-suspenders; the length check covers the load-bearing contract.

Neither observation is a substantive finding under the "zero findings" bar — both leave the test correct, the spec citation defensible, and the coverage goal met.

### New findings

**None.** Every scrutinize item resolved with in-source, in-spec, or in-run evidence. Constructor guards have exact-message regex matches. The `listProfiles` mutation test correctly distinguishes a fresh-array impl from an internal-leak impl. `addProfile`'s replace semantic is pinned by two complementary assertions (overwritten-value + size-unchanged). Seed argument is a string per the type declaration; no stale numeric-seed test remains. `assert.equal` on `getWallet` is reference-identity via `Object.is`. R-citations are defensible (R12 is exact; R11 is broader-than-tested but not lying — soft observation only). Comments are load-bearing. Tests are independent. `profiles.ts` hits 100/100/100 — full closure of P5.

### Verdict: **PASS**

`profileRegistry.test.ts` closes P5 cleanly: 12 tests, all passing, drive `profiles.ts` from 84.92% line / 75% branch / 66.67% function to 100% across all three dimensions. The test contract pins SPEC-0001 R12 profile-switching behavior (constructor defaults + custom-list + activeId honoring + throw-on-empty + throw-on-unknown-activeId + identity wallet retention + active-partyId delegation + fresh-array `listProfiles` + flip-and-throw `setActiveProfile` with atomic-on-failure + replace-not-duplicate `addProfile` + miss-returns-undefined `getProfile`). Constructor guards are tested with regex matches anchored to the exact thrown wording. The fresh-array contract is genuinely tested (a leaky impl would fail the length assertion). The replace semantic is pinned by overwrite + size-unchanged. Tests are independent, the shared seed is deterministic-not-stateful, and the wallet identity check uses `Object.is`. Two soft observations surface (R11-citation is broader-than-tested; the freshness test omits an array-identity check) but neither is substantive under the zero-findings bar. Hardhat / web / contract layers untouched this tick. Fit to commit.

## Tick 62 strict-review

**Date:** 2026-05-30 · **Branch:** `spec-4-implementation` · **HEAD vs:** `b1cfbcc`
**Diff:** 1 new file — `src/wallet/wallet.test.ts` (~165 lines, 13 tests).
**Claim:** closes coverage gap **P6** — `wallet.ts` was at 0% line / 0% branch / 0% function (coverage.md L63), queued threshold `line >= 85% (RealWallet exempt — requires live provider)` (coverage.md L99).

### Empirical re-verification

- `npx tsx --test src/wallet/wallet.test.ts` -> **13/13 pass** (duration ~670 ms).
- `npx tsx --test 'src/**/*.test.ts'` -> **160/160 pass** (matches the +13 claim over tick-61's 147/147 baseline).
- `node --import tsx --test --experimental-test-coverage --test-coverage-include='src/wallet/wallet.ts' src/wallet/wallet.test.ts` -> `wallet.ts` reports **89.04% line / 84.00% branch / 77.78% function**. Uncovered lines reported by V8: **12-19** (file-header docblock close + import statements — non-executable in any meaningful sense, but the V8 counter doesn't elide them), **30-36** (`Wallet` interface body — type-only, elided at runtime), **52** (a docblock-comment line above the `SimulatedWallet` constructor body). The **`RealWallet` class body (L62-83) is the only genuine executable-line gap** and is by-design exempt per the queued P6 threshold. P6 acceptance criterion was `line >= 85%` -> **89.04% exceeds it by ~4pp**; every executable branch reachable without a live provider is covered.

### Scrutinize-for-zero-findings checklist (per tick-62 prompt)

| # | Item | Finding |
|---|------|---------|
| 1 | **No-mock invariant — `RealWallet` exemption sound?** | **Sound.** The loop-prompt invariant prohibits **mocking the database / contract / agent** in tests that claim integration coverage. `RealWallet`'s constructor instantiates `ethers.JsonRpcProvider` (which performs no I/O at construction — it only stores the URL + chainId) and `ethers.Wallet` (pure-key wrapper). In principle, constructing `RealWallet` with `http://127.0.0.1:0` + a real-shape key would not violate the no-mock rule because no chain RPC is mocked — the provider just never gets called. **But** the broader spirit of the invariant — and the explicit P6 queue text "RealWallet exempt — requires live provider" — interprets the rule as "any code path that uses ethers' RPC-bound machinery is verified against real Somnia testnet, not synthetically wired in a unit test." Per `docs/progress/browser-verify.md`, RealWallet is exercised end-to-end against the deployed contract on Somnia testnet — that's the canonical coverage path. The file header at L1-11 acknowledges the exemption with the correct rationale: "RealWallet construction is exempt from unit testing per the loop's no-mock invariant — it requires an actual ethers JsonRpcProvider + a real private key and is exercised via the testnet-mode browser-verify harness instead." Inspection confirms the test only touches RealWallet via the synchronous validation guards in `createWallet` (missing key, malformed key, short key); none of those guards reach `new RealWallet(...)` because the guards throw first. **No finding.** |
| 2 | **`withEnv` helper safety — restores env on throw?** | **Correct — try/finally restoration is sound.** At L37-49 the helper saves the original values into `saved` **before** any mutation, then in a `try { mutate; fn() } finally { restore }` block restores every key the caller asked to override (not every key in process.env). The `finally` runs whether `fn()` returns normally or throws (the helper signature is sync-only: `fn: () => void` at L32). The restoration loop at L44-48 iterates the **override keys** specifically, not the saved entries — but `saved` was populated from the same override keys at L36, so both sets are equivalent. Edge case: if `overrides` contained a duplicate key, the second occurrence's save would overwrite the first — but `Object.entries` on a plain object cannot yield duplicates because object keys are unique. **No finding.** |
| 3 | **Process.env mutation isolation — node:test parallel risk?** | **Safe — node:test runs tests within a single file serially by default.** node:test (the Node.js built-in test runner) defaults to **`concurrency: 1`** for top-level `test()` calls in a single file — they run sequentially in declaration order. Concurrency must be explicitly opted into via `test('name', { concurrency: true }, ...)` or `test.concurrent(...)`. No test in this file opts in. Across files, node:test runs **separate test files in separate child processes** (when invoked with `--test path/to/**`), so process.env in one file's process cannot leak into another. Within this file: tests run serially, `withEnv` restores in `finally`, the L161-165 sanity test confirms ORIGINAL_ENV matches the post-suite snapshot. The ORIGINAL_ENV snapshot at L24-28 is captured at module-load time before any test runs — the canonical "host-env" baseline. **No finding.** |
| 4 | **Address-computation independence — self-pinning (tick-49 M2 class)?** | **Defensible — pins the wire-format contract, not the impl.** The test at L63-70 recomputes `ethers.getAddress("0x" + ethers.keccak256(ethers.toUtf8Bytes(seed)).slice(-40))` and asserts equality with `w.address`. The impl at wallet.ts L52-54 does **exactly the same computation** — so taken alone this is a tautology. But it's positioned as a **wire-format pin**: it pins that the address derivation is (a) keccak256-based, (b) UTF-8-encoded seed input, (c) trailing-20-bytes truncation, (d) EIP-55 checksum-cased. A future impl change that swapped to a different hash (sha256), a different encoding (latin1), a different truncation (leading bytes), or unchecksummed lowercase output would fail this test. The risk it doesn't catch is a coordinated impl + test edit. The mitigation is the **complementary** trio at L72-76 (same seed -> same address), L78-82 (different seeds -> different addresses), and L84-89 (default-seed -> valid checksummed 40-hex). Together the four tests pin structure + determinism + distinguishability + the specific keccak256-trailing-20 contract. **Soft observation:** adding `assert.equal(new SimulatedWallet("seed-A").address, "0x...literal...")` would belt-and-suspenders the self-pin risk. The current form is defensible (the file header at L11 establishes this test's intent as "verified independently via ethers") but it's at the edge of the M2 class. **Soft observation, not a finding.** |
| 5 | **Determinism test — same-seed -> same-address.** | **Strong.** L72-76 constructs two `SimulatedWallet`s with `"repeat-seed"` and asserts `a.address === b.address`. Catches any nondeterminism introduced by impl drift — a Date.now()/Math.random() leak, a counter, an instance-id mixed into the seed. Independent of the wire-format pin (#4), this test catches "deterministic-but-different-algorithm" only if the algorithm change preserved determinism for all seeds — but combined with #4 the contract is tight. **No finding.** |
| 6 | **Default-seed sanity — non-zero address is enough?** | **Adequate-for-its-role.** L84-89 constructs `new SimulatedWallet()` (no seed), asserts the address matches `/^0x[0-9a-fA-F]{40}$/` and is not all-zero. A future change to the default seed (e.g. `"curie-simulated-wallet"` -> `"curie-default-wallet"`) would change the derived address but not trip this test (the new address would still be 40-hex and non-zero). **Soft observation:** pinning the literal address derived from the current default seed would make a default-seed change loud. But the default seed is a low-stakes constant (changing it doesn't affect production correctness — only demo display); the role here is shape validation, and pinning bytes would over-tighten. **Soft observation, not a finding.** |
| 7 | **Malformed-mode test — regex catches "garbage"?** | **Pinned correctly.** `resolveMode` at wallet.ts L102-110: reads `process.env.SOMNIA_WALLET_MODE`, trims, lowercases; if `"real"` returns "real", if `"simulated"` returns "simulated", **else throws** `` `Invalid SOMNIA_WALLET_MODE="${process.env.SOMNIA_WALLET_MODE}". Expected "simulated" or "real".` ``. For `SOMNIA_WALLET_MODE: "garbage"`: the trimmed-lowercased `"garbage"` matches neither branch and hits the throw. The test regex `/Invalid SOMNIA_WALLET_MODE/` matches the literal substring at the start of the message. The throw quotes the **un-trimmed un-lowercased** env value (L108 reads `process.env.SOMNIA_WALLET_MODE`, not the locally-lowercased `raw`) — so `"garbage"` is included verbatim, which is correct error-message UX. **No finding.** |
| 8 | **Missing-key test — host-env leak?** | **Safe — `withEnv` unsets PRIVATE_KEY for the duration.** L137-144: `withEnv({ PRIVATE_KEY: undefined }, () => createWallet({ mode: "real" }))`. In `withEnv` at L38-41, `v === undefined` triggers `delete process.env.PRIVATE_KEY` — so even if the host-env has `PRIVATE_KEY=0x...` set at module-load, the delete clears it before `fn()` runs. After `fn()`, the `finally` at L44-48 restores the host's original value via the saved snapshot at L36. The createWallet call sees `options.privateKey === undefined` (no explicit override) AND `process.env.PRIVATE_KEY === undefined` (deleted), hits the L134 guard, throws. The regex `/Real wallet mode requires a private key/` matches the literal at wallet.ts L136. **No finding.** |
| 9 | **Comment quality + R-citations.** | **All load-bearing; SPEC-0001 R11 citation is accurate.** Header L1-11 cites SPEC-0001 R11 and explains the RealWallet exemption rationale. SPEC-0001 R11 at L62 of the spec: "Each request registers a **provider address** and an **insurer address**. Every party action is gated `msg.sender ∈ {providerAddr, insurerAddr}`…" — R11 is **wallet auth** (gating party actions to registered addresses). `SimulatedWallet` and `createWallet` are the modules that produce the **address** registered + used for the auth check; R11 is the correct anchor for the wallet abstraction. The impl module at wallet.ts L2 also cites "SPEC-0001 R11/R12" — alignment between test and impl headers. R12 (single-shared-wallet semantics) is not directly cited in the test header but is implicitly covered by the deterministic-address tests (which let two profiles share the same wallet for the single-wallet demo). Section dividers at L19-22 ("Reset SOMNIA_WALLET_MODE / PRIVATE_KEY between tests"), L52-54 ("SimulatedWallet"), L91-93 ("createWallet — simulated branch"), L131-135 ("createWallet — real-mode error guards" + the no-mock rationale recap) group tests by surface — aid navigation, no padding. The L86-87 comment names the checksummed-20-byte-hex shape contract. L131-135 recaps the no-mock exemption rationale at the section divider — anchors the reader at the spot where the rationale is load-bearing. No comments restate code, none lie about citations. **No finding.** |
| 10 | **Coverage delta — close P6 in `coverage.md`?** | **Yes, P6 closes.** Pre-tick: `wallet.ts` 0% / 0% / 0% (coverage.md L63). Post-tick (tool-verified above): **89.04% line / 84.00% branch / 77.78% function**. P6 acceptance criterion (coverage.md L99): `line >= 85% (RealWallet exempt — requires live provider)` -> exceeded by ~4pp. The remaining uncovered cells (the `RealWallet` constructor body L62-83 + V8-reported but non-executable lines: imports, the `Wallet` interface, a docblock-comment line) are the documented exemption: RealWallet requires a live ethers JsonRpcProvider and is verified via testnet-mode browser-verify, not synthetic unit tests. Per the tick-58/59/60/61 precedent (defer the per-item closure annotation in coverage.md to a bookkeeping tick), the queue text at L99 remains accurate for the pre-tick state; the closure is recorded here. **No finding.** |

### Soft observations (not findings)

- **Self-pin risk on the wire-format test** (item 4): the trailing-20-bytes-of-keccak256 test recomputes via the same ethers primitives the impl uses. The complementary determinism + distinguishability + shape tests mitigate the M2 risk; pinning a literal byte-address for a fixed seed would belt-and-suspenders this. Surfacing for the tick-49 M2 register.
- **Default-seed address not byte-pinned** (item 6): the default-seed test pins shape + non-zero but not the literal address. If the default seed becomes a demo-stable identifier, pinning the literal bytes (one extra `assert.equal`) would catch a default-seed drift loud.

Neither observation is substantive under the "zero findings" bar — both leave the test correct, the spec citation accurate, and the coverage goal met.

### New findings

**None.** Every scrutinize item resolved with in-source, in-spec, or in-run evidence. The RealWallet exemption rationale is sound (no-mock invariant + testnet-mode browser-verify covers the gap). The `withEnv` helper restores on throw via try/finally. node:test runs tests serially within a file by default — process.env mutation cannot race. The address-computation test is at the edge of the M2 self-pin class but the complementary determinism + distinguishability + shape tests mitigate it. The malformed-mode regex matches the exact thrown wording. The missing-key test explicitly unsets PRIVATE_KEY so host-env cannot leak in. R11 is the correct anchor for the wallet abstraction. `wallet.ts` hits 89.04% line / 84% branch — above the P6 `>= 85%` threshold — with the only uncovered executable lines being the `RealWallet` constructor body, explicitly exempt per the P6 queue text.
...
## Tick 81 (278fd33 R12 reactive pill-row)

**Scope.** Commit `278fd33` — "feat(web): SPEC-0005 R12 — reactive pill-row from userStore (no reload)". Six files: `src/profiles/profiles.ts` (+21), `src/profiles/profileRegistry.test.ts` (+45), `web/src/App.tsx` (+44/−4), `web/src/client.ts` (+76), `web/src/views/Settings.tsx` (+19/−4), `web/tests/agent-browser/run.sh` (+19/−2). Strict-stickler brief: zero findings required.

### Verification gates

| Gate | Result |
|---|---|
| `npx tsc -p tsconfig.json` | PASS — clean, no diagnostics. |
| `cd web && npx tsc --noEmit` | PASS — clean, no diagnostics. |
| `npm run test:lib` | PASS — 196/196 (including 4 new `removeProfile` tests at `src/profiles/profileRegistry.test.ts:146-185`). |
| Harness Scenario I (R12 add/remove asserts) | PASS — claim reported; the 16 other harness scenarios that fail do so because `.env` is real-mode + the insurer wallet `0x140e…8C62` is unfunded (R18 operator-blocker), not regressions from this tick. |

### Lines of review

**Q1. React effect correctness — clean.** `App.tsx:68-87` registers the `USERS_CHANGED_EVENT` + `storage` listeners with symmetric `removeEventListener` cleanup inside the return; empty deps `[]` is correct because every closure captures only module-level imports (`syncProfilesFromUsers`, `loadUsers`, `USERS_STORAGE_KEY`, `USERS_CHANGED_EVENT`) or stable React setState dispatchers (`setActiveProfileId`, `setProfilesEpoch`), both of which React guarantees identity for across renders (React docs, "useState returns a setter with stable identity"). No stale-closure trap. The functional updater `setProfilesEpoch((n) => n + 1)` at `App.tsx:75` correctly avoids reading a stale `profilesEpoch`. Under `StrictMode` (confirmed on at `web/src/main.tsx:11`) the effect's setup→cleanup→setup→cleanup double-invocation is balanced: each `addEventListener` is paired with a `removeEventListener` for the same function reference inside the same closure, so the double-mount in dev does not leak listeners.

**Q2. Race between `dispatchEvent` and `loadUsers` — impossible by construction.** `Settings.tsx:375-387` runs `saveUsers(next)` (a synchronous `storage.setItem(USERS_STORAGE_KEY, JSON.stringify(users))` per `src/users/userStore.ts:119-130`) BEFORE `window.dispatchEvent(new CustomEvent(USERS_CHANGED_EVENT))`. `dispatchEvent` invokes listeners synchronously per the DOM spec; the listener's `loadUsers()` call reads from storage which was already written. The "yesterday's snapshot" race the brief asks about cannot occur: there is no async boundary between the write and the read.

**Q3. `void profilesEpoch;` reference — defensive documentation, not idiomatic-but-also-not-wrong.** `tsconfig.json` and `web/tsconfig.json` do not set `noUnusedLocals`, and no ESLint config exists (`ls web/.eslintrc*` / `web/eslint.config.js` empty). So `profilesEpoch` would not actually warn as unused. The `void profilesEpoch;` line is therefore not load-bearing for any lint pass — it functions as a self-documenting "do not delete this state slot" marker for readers. The re-render itself is triggered by `setProfilesEpoch((n) => n + 1)` (React's setState-triggers-re-render contract); the explicit reference is unnecessary for correctness. A cleaner alternative would have been `const profiles = useMemo(() => client.profiles.listProfiles(), [profilesEpoch])` — which makes `profilesEpoch` a real dep — but the present pattern works because the bare `const profiles = client.profiles.listProfiles()` at `App.tsx:92` re-runs on every render and React re-renders whenever any state changes. The `useMemo` at L93-97 lists `profilesEpoch` as a dep correctly. **Not a finding** — it is harmless documentation that survives a future `noUnusedLocals` adoption.

**Q4. `useMemo` deps `[activeProfileId, profilesEpoch]` — correct.** `getActiveProfile()` returns `this.profiles.get(this.activeId)`. The result changes when (a) the active id flips (`setActiveProfile`) — captured by `activeProfileId`; or (b) the registry's map for the current active id changes — `addProfile` replaces it (covered by `profilesEpoch`), `removeProfile` of a non-active id doesn't affect the active result (also covered safely). The deps cover both axes. The eslint-disable comment was inherited from the pre-tick state where only `activeProfileId` was listed; the comment is now technically over-broad (both deps ARE listed) but the inherited disable doesn't lie about the new deps — it just remains because the closure references `client.profiles` which is module-level and not listed. Not a finding.

**Q5. `syncProfilesFromUsers` semantics — correct under SEED_PROFILE_IDS invariant.** Order of operations at `web/src/client.ts:335-368` is: build `desired` (non-seed users from userStore) → iterate current non-seed registry entries → for orphans, if active, vacate to "provider" via `setActiveProfile("provider")` + `setActiveClientProfile("provider")` (paired so the SimulatedBackend's `caller` flips too) → call `removeProfile` → record removal. Then iterate `desired` → for each id not in registry, `addProfile(userToProfile(user))`. The fallback to literal "provider" is safe because (i) "provider" is in `SEED_PROFILE_IDS` (`client.ts:303-307`), (ii) `makeClient` always seeds it first (`client.ts:144-153`), (iii) `currentNonSeed` filters seeds out of the iteration so "provider" is never a removal candidate, (iv) the slug-collision path is closed: any DemoUser with `id === "provider"` is filtered out of `desired` by `users.filter((u) => !SEED_PROFILE_IDS.has(u.id))` at `client.ts:339`, and `makeClient` similarly drops persisted seed-id collisions at boot at `client.ts:148-151`. The fallback "provider" literal is hardcoded in three places (`client.ts:303` in SEED_PROFILE_IDS, `client.ts:351` in syncProfilesFromUsers, `App.tsx:72` in the activeWasRemoved branch) which couples them; a future rename of the seed id would need a coordinated update at all three. The coupling is honest — DEFAULT_PROFILES at `src/profiles/profiles.ts:42-55` is the canonical seed source — and consistent with how the codebase has been treating seed identity since T75b. **Not a finding.**

**Q6. Mid-reconcile state — atomic per call.** The function has no `await`s, no microtask boundaries, no nested event re-entry — it is a single synchronous traversal. The transient state between `setActiveProfile("provider")` and `removeProfile(p.id)` (active id moved, old entry not yet deleted) is invisible to any other JS code. Safe.

**Q7. `removeProfile` API surface — clean.**
- *Active-id guard message.* `\`Cannot remove the active profile "${id}" — switch first.\`` is clear and prescriptive; mirrors the existing `setActiveProfile` throw pattern at `src/profiles/profiles.ts:111`. Em-dash matches the codebase voice.
- *Error subclass.* Plain `Error` — consistent with the rest of `ProfileRegistry` (constructor + `setActiveProfile` both throw plain `Error`). A subclass would be premature abstraction here; the caller (`syncProfilesFromUsers`) never catches.
- *Return type.* `boolean` reflects `Map.prototype.delete`'s signature; after the throw-guards the call always returns `true`, so `void` would also have been honest, but exposing the underlying Map's return type isn't wrong. NIT-not-finding.
- *Implicit emptiness guard.* The doc comment at L122-130 explicitly names this: "a single-entry registry must have that entry active." The fourth test at L172-185 makes this implicit invariant explicit. Honest design.

**Q8. Test coverage — exercises real paths, not just error strings.**
- L146-153 (happy): asserts return `true`, post-state `getProfile` is `undefined`, and length math. Real state change verified.
- L155-158 (unknown id): regex `/Unknown profile "ghost"/` covers both the throw and the message format.
- L160-170 (active-id refusal): post-throw asserts `getProfile("observer")?.id === "observer"` — i.e. **the registry is unchanged after the rejected delete**. This is the impl invariant, not just the error string. Real path coverage.
- L172-185 (single-entry stays active): constructs a one-entry registry, asserts the active-profile-guard fires before any emptiness check. Documents and pins the implicit-emptiness-via-active-guard observation; would catch a future regression like a `forceRemove` that bypassed the guard. Defensible.

**Q9. Settings dispatch — deliberate design, not over-engineering.** `Settings.tsx:384-386` guards `typeof window !== "undefined"` (defensive even though Settings is browser-only). The deliberate choice NOT to pass a `detail` payload is documented in the L379-383 comment: same-tab CustomEvents and cross-tab `storage` events flow through the same `reconcile()` code path so behavior is uniform regardless of source. Round-trip cost: localStorage read + JSON parse of a small array per dispatch — trivial. The pattern is correct.

**Q10. Harness assertions — React-compatible setter pattern, consistent timing.** L486-488 use the canonical `Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set.call(el, value)` then `dispatchEvent(new Event('input', {bubbles:true}))` — this IS the React-controlled-input pattern (React tracks the input's `value` via its own prototype-setter hook, so a direct `el.value = ...` would not register; the prototype setter does). Identical pattern used in Stripe Elements tests, react-testing-library workarounds, etc. The R19 select uses `'change'` correctly. `ab wait 200` after submit matches at least 12 other `ab wait 200` calls in the same file (`grep "ab wait 200"` → 12 hits at L157, L187, L189, L193, L263, L315, L340, L394, L490, L496 + earlier). The 200ms covers React's microtask flush + reconcile + DOM commit; the rest of the harness uses the same envelope without observed flake. Consistent.

**Q11. Hint text — accurate.** "SPEC-0005 R10/R11/R12: arbitrary N users persisted to `curie:users` in localStorage. The seed roles (provider / insurer / observer) remain available in the picker above; entries added here appear as additional profile pills in the top-bar immediately (R12 — no page reload required)." R10 (arbitrary N) — satisfied. R11 (runtime add/remove) — satisfied. R12 ("Role pill row reflects the full registry, not a hardcoded list") — now satisfied reactively. "Immediately" is honest given the synchronous dispatch+listener+setState pipeline. The wording does not lie about scope.

**Q12. Spec drift — none.** SPEC-0005 R12 at `docs/specs/0005-usability-and-integration.md:81-83`: "Role assignment per user. Roles are `provider` | `insurer` | `observer` (extensible). The Role pill row reflects the full registry, not a hardcoded list." The pill row at `App.tsx:189-205` maps over `profiles = client.profiles.listProfiles()` and reactively re-derives on userStore changes via the new useEffect. The `DemoRole` type is `"provider" | "insurer" | "observer"`; `partyIdFor` at `client.ts:116-125` is an exhaustive switch (tsc-enforced under `strict`). "(extensible)" in the spec is honestly compile-time-extensible: adding a role needs one type-union extension + one switch case. Not runtime-extensible by design (the type union is closed for v0); the spec's parenthetical "(extensible)" is correctly interpreted as the design-time extensibility this code allows. No drift.

**Q13. Over-engineering / dead code / backwards-compat hacks.**
- `ProfileSyncOutcome` has three fields (`added`, `removed`, `activeWasRemoved`); App uses all three (`outcome.activeWasRemoved` directly, `outcome.added.length || outcome.removed.length` as a single change-bit). The richer shape is useful for future logging and is reasonable surface. NIT.
- `SEED_PROFILE_IDS` constant duplicates info already in `DEFAULT_PROFILES + "observer"`. Could be derived. But the duplication is intentional (the seed set is genuinely fixed for v0 and the comment names the relationship), the cost is a 3-string Set, and consolidating would require routing DEFAULT_PROFILES through a different code path. NIT-not-finding.
- No backwards-compat hacks introduced. No dead code.
- `void profilesEpoch;` analyzed in Q3 — defensive documentation.
