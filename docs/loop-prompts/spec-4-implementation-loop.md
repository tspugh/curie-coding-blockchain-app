...
1. **Tests** — `npm test` (chain defined in `package.json` `scripts.test`, in order: `check-ruling-abi` → `typecheck` → `test:lib` → hardhat — fastest-first; ~19s end-to-end) + web E2E. The `check-ruling-abi` gate parses `CoverageNegotiation.sol`'s `abi.decode(responses[0].result, (TYPES))` call at runtime via regex and asserts the orchestrator's `scripts/lib/ruling-abi.ts` encoder matches that live shape (not a hand-copied literal), then round-trips 5 sample rulings end-to-end through the ethers ABI codec — SPEC-0004 R26 repurposed under Amendment 0006, see [`../amendments/0006-self-hosted-arbiter-agent.md`](../amendments/0006-self-hosted-arbiter-agent.md). Cheap, chain-independent, catches drift introduced by changes to either side. The hardhat suite additionally exercises the mirror linkage end-to-end via the `orchestrator handleResponse round-trip` test, which imports the orchestrator's actual `encodeRuling` and delivers the bytes through the compiled Solidity decoder. **100% pass.**
